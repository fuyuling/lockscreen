...
public class SharedPreferencesUtil {
    ...
    public static void init(Context context) {
        if(context == null){ Log.d("Lynn", "cpmtext =nulll");}
        mPref = context.getSharedPreferences(NAME, Context.MODE_PRIVATE);
    }

    public static String get(Context context, Cmd cmd) {
        if (mPref == null){
            Log.d("Lynn", "mPref == null");
            mPref = context.getSharedPreferences(NAME, Context.MODE_PRIVATE);
        }
        if(!mPref.contains(cmd.name())) {
    ...
    public static boolean get(Context context, String _key) {
        if (mPref == null){
            Log.d("Lynn", "mPref == null");
            mPref = context.getSharedPreferences(NAME, Context.MODE_PRIVATE);
        }
        if(!mPref.contains(_key)) {
